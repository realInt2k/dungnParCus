...
void AngoraLLVMPass::visitCompareFunc(Instruction *Inst) {
  ...
  //errs() << "in cmpfunc5 "; ArgSize->getType()->print(errs()); errs() << "\n";
  ArgSize = IRB.CreateZExtOrTrunc(ArgSize, Int32Ty, "int2k cast i32 or trunc");
  /*errs()<<"\n\nIn the visit compare func\n";
  errs()<<"Cid "; Cid->getType()->print(errs()); errs() << "\n";
  errs()<<"CurCtx "; CurCtx->getType()->print(errs()); errs() << "\n";
  errs()<<"CallSite "; CallSite->getType()->print(errs()); errs() << "\n";
  errs()<<"ArgSize "; ArgSize->getType()->print(errs()); errs() << "\n";
  errs()<<"OpArg[0] "; OpArg[0]->getType()->print(errs()); errs() << "\n";
  errs()<<"OpArg[1] "; OpArg[1]->getType()->print(errs()); errs() << "\n"; */
  CallInst *ProxyCall =
...
void AngoraLLVMPass::processCmp(Instruction *Cond, Constant *Cid,
  ...
  if (FastMode) {
    ...
    */
    ...
    setInsNonSan(CurCtx);
/*
errs()<<"\n\nIn the ProcessCmp ThenB.Call(TraceCmp, {CondExt, Cid, CurCtx, OpArg[0], OpArg[1]}); \n";
errs()<<"CondExt "; CondExt->getType()->print(errs()); errs() << "\n";
errs()<<"Cid "; Cid->getType()->print(errs()); errs() << "\n";
errs()<<"CurCtx "; CurCtx->getType()->print(errs()); errs() << "\n";
errs()<<"OpArg[0] "; OpArg[0]->getType()->print(errs()); errs() << "\n";
errs()<<"OpArg[1] "; OpArg[1]->getType()->print(errs()); errs() << "\n";
*/
    ...
    LoadInst *CallSite = IRB.CreateLoad(ParmeSanIndCallSite);
    setInsNonSan(CallSite);
    /*errs() << "In the ProcessCmp\n";
    errs() << "Cid "; Cid->getType()->print(errs()); errs() << "\n";
    errs() << "CurCtx"; CurCtx->getType()->print(errs()); errs() << "\n";
    errs() << "CallSite "; CallSite->getType()->print(errs()); errs() << "\n";
    errs() << "SizeArg "; SizeArg->getType()->print(errs()); errs() << "\n";
    errs() << "TypeArg "; TypeArg->getType()->print(errs()); errs() << "\n";
    errs() << "OpArg[0] "; OpArg[0]->getType()->print(errs()); errs() <<"\n";
    errs() << "OpArg[1] "; OpArg[1]->getType()->print(errs()); errs() <<"\n";
    errs() << "CondExt "; CondExt->getType()->print(errs()); errs() <<"\n";*/
    CallInst *ProxyCall =
...
void AngoraLLVMPass::processBoolCmp(Value *Cond, Constant *Cid,
  ...
  //errs() << "it runs fine in process bool cmp\n"; // int2k;
}
...
void AngoraLLVMPass::visitSwitchInst(Module &M, Instruction *Inst) {
  ...
  if (FastMode) {
    ...
    */
    ...
    for (auto It : Sw->cases()) {
      ...
      //errs() << C->getType()->getScalarSizeInBits() << "\n";
      //errs() << Int64Ty->getScalarSizeInBits() << "\n";
      if (C->getType()->getScalarSizeInBits() >= Int64Ty->getScalarSizeInBits())
        continue; // int2k fix here
      ArgList.push_back(ConstantExpr::getCast(CastInst::ZExt, C, Int64Ty));
      //ArgList.push_back(ConstantExpr::getTruncOrBitCast(C, Int64Ty));
    }
    ...
    setInsNonSan(CallSite);
    /*
    errs() << "\n\n Visit switch inst TraceSwTT \n";
    errs() << "Cid "; Cid->getType()->print(errs()); errs() << "\n";
    errs() << "CurCtx"; CurCtx->getType()->print(errs()); errs() << "\n";
    errs() << "CallSite "; CallSite->getType()->print(errs()); errs() << "\n";
    errs() << "SizeArg "; SizeArg->getType()->print(errs()); errs() << "\n";
    errs() << "CondExt "; CondExt->getType()->print(errs()); errs() <<"\n";
    errs() << "SwNum "; SwNum->getType()->print(errs()); errs() << "\n";
    errs() << "ArrPtr "; ArrPtr->getType()->print(errs()); errs() << "\n";
    */
  ...
  //errs() << "it runs fine visitSwitchInst \n";
}
      ...
      if (ParamType->isIntegerTy() || ParamType->isPointerTy()) {
        if (!isa<ConstantInt>(ParamVal)) {
          ...
          if (ParamType->isPointerTy()) {
            ParamVal = IRB.CreatePtrToInt(ParamVal, Int64Ty, "int2kFixLine895AngoraPass");
            size = 8;
          ...
          if (TrackMode) {
            ...
            setInsNonSan(CallSite);
            /*
            errs() << "\n INT2k Cid "; Cid->getType()->print(errs()); 
            errs() << "\n INT2k CurCtx "; CurCtx->getType()->print(errs());
            errs() << "\n INT2k CallSite "; CallSite->getType()->print(errs());
            errs() << "\n INT2k SizeArg "; SizeArg->getType()->print(errs());
            errs() << "\n INT2k TypeArg "; TypeArg->getType()->print(errs());
            errs() << "\n INT2k ParamVal "; ParamVal->getType()->print(errs());
            */
            CallInst *ProxyCall = IRB.CreateCall(
...
bool AngoraLLVMPass::runOnModule(Module &M) {
  ...
  //errs() << "int2k: ok RunOnModule\n";

  if (is_bc)
    OKF("Max constraint id is %d", CidCounter);
  errs() << "Angora pass it finishes\n";
  return true;
}
