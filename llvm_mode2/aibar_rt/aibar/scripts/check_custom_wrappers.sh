...
AIBAR_DIR=$(dirname "$0")/../
AIBAR_CUSTOM_TESTS=${AIBAR_DIR}/../../test/aibar/custom.cc
AIBAR_CUSTOM_WRAPPERS=${AIBAR_DIR}/aibar_custom.cc
AIBAR_ABI_LIST=${AIBAR_DIR}/done_abilist.txt
...
trap on_exit EXIT
grep -E "^fun:.*=custom" ${AIBAR_ABI_LIST} \
  | grep -v "aibar_get_label\|__sanitizer_cov_trace" \
  | sed "s/^fun:\(.*\)=custom.*/\1/" | sort > $DIFF_A
grep -E "__dfsw.*\(" ${AIBAR_CUSTOM_WRAPPERS} \
...
grep -E __dfsw_ ${AIBAR_CUSTOM_WRAPPERS} \
  | grep -v "__sanitizer_cov_trace" \
  | sed "s/.*__dfsw_\([^(]*\).*/\1/" | sort > $DIFF_A
grep -E "^[[:space:]]*test_.*\(\);" ${AIBAR_CUSTOM_TESTS} \
